...
def load_checkpoint(model, weights):
    checkpoint = torch.load(weights,map_location=torch.device('cpu'))#,"map_location=torch.device('cpu')" use this mapping only if cuda is unavailable
    try:
...
def get_arch(opt):
    ...
    print('Architecture Loaded: '+arch+'...')
    if arch == 'UNet':
